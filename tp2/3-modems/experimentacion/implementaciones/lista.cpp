...
struct DSU{
    ...
    DSU(int n){
        elementos = vector<nodoDsu>(n+1);
        for (int i=1; i <= n; i++) elementos[i] = {i, i};
    }

    int find(int v){
        auto temp = elementos[v];
        while (temp.data != temp.head) {
            temp = elementos[temp.head];
        }
        return temp.head;
    }
    ...
    void unite(int u, int v){
        u = find(u), v = find(v);
        if(u == v) return;
        elementos[v].head = elementos[u].head;
    }

    vector<nodoDsu> elementos;
};
...
int main() {
    ...
    output_file.open("runtime-data/runtime-lista.csv", std::ios_base::out);
