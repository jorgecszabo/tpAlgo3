...
int main() {
    ...
    output_file.open("runtime-data/runtime-path-compression.csv", std::ios_base::out);
    ...
    for (int caso = 1; caso <= casos; caso++) {
        ...
        for (auto i=0; i < repeticiones; i++) {
            auto t1 = c.now();
            tuple<double, double> res = solve(listaAdj, n, cantModems, precioUTP, limitUTP, precioFibra);
            auto t2 = c.now();
